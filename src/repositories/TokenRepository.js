...
async function upsertShop(shopId, region) {
  return prisma.shop.upsert({
    ...
    create: { shopId: BigInt(shopId), region: region || null, status: "AUTHORIZED" }
  });
}

async function saveTokens({ shopId, accessToken, accessExpiresIn, refreshToken, refreshExpiresIn }) {
  const shop = await upsertShop(shopId);
